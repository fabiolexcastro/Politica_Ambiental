...
# Load data ---------------------------------------------------------------
fls <- list.files('../tif/casanare_v2/chirps', full.names = TRUE, pattern = '.tif$')
yrs <- 1983:2016
...
aggregated_data_month <- function(yr){
  ...
  df <- as_tibble(df)  
  df <- df %>% mutate(month = as.numeric(str_sub(file, 18, 19)))
  ...
  Map('writeRaster', x = rs, filename = paste0('../tif/casanare_v2/chirps/monthly/chirps_', yr, '_', 1:12, '.tif'), overwrite = TRUE)
  print('Done!')
